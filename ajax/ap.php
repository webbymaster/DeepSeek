...
if ($first == 'save-settings') {
    ...
    if (!empty($submit_data)) {
        ...
        foreach ($submit_data as $key => $value) {
            ...
            if ($key == 'who_can_donate') {
                if ($value == 'admin') {
                    $db->where('admin', 1,'!=')->update(T_USERS,['donation_link' => '']);
                }
                elseif ($value == 'pro') {
                    $db->where('admin', 1,'!=')->where('is_pro', 0)->update(T_USERS,['donation_link' => '']);
                }
            }
            if ($key == 'bank' || $key == 'p_link' || $key == 'link' || $key == 'custom') {
                if (in_array($value, array(0,1))) {
                    $p_key = $key;
                    if ($key == 'p_link') {
                        $p_key = 'link';
...
if ($first == 'create-ads') {
    if (!empty($_POST['type'])) {
        if ((empty($_POST['name']) || empty($_POST['link']) || empty($_POST['ad_link']) || empty($_POST['type'])) && $_POST['type'] != 'vast') {
            ...
            if ($_POST['type'] == 'video') {
                if (!preg_match("([^\s]+(\.(?i)(mp4|webp|mp3|mpeg|mov|3gp|avi|flv|ogg|mkv|mk3d|mks|wmv|m4v))$)", $_POST['link'])) {
...
if ($first == 'submit-sitemap-settings') {
    ...
    if ($_POST['completed'] == 0) {
        ...
        if ($percentage >= 100) {
            ...
            $data['last_created'] = date('d.m.Y / H:i:s');
...
if ($first == 'clean_videos') {
    if (in_array($_GET['selected_time'], array('all','today','this_week','this_month','this_year'))) {
        ...
        if ($_GET['selected_time'] != 'all' && !empty($time_start) && !empty($time_end)) {
            ...
            $where .= " AND (`youtube` <> '' OR `vimeo` <> '' OR `daily` <> '' OR `ok` <> '' OR `vk` <> '' OR `rutube` <> '' OR `dzen` <> '')";
        } else {
          $where .= " (`youtube` <> '' OR `vimeo` <> '' OR `daily`  <> '' OR `ok` <> '' OR `vk` <> '' OR `rutube` <> '' OR `dzen` <> '')";
        }
...
//             'credentials' => array(
//                 'key' => $pt->config->yandex_key_2,
//                 'secret' => $pt->config->yandex_secret_2
//             )
//         ));

//         $buckets  = $s3Client->listBuckets();
//         if (!empty($buckets)) {
//             if ($s3Client->doesBucketExist($pt->config->yandex_name_2)) {
//                 $data['status'] = 200;
//             } else {
//                 $data['status'] = 300;
//             }
...
if ($first == 'ffmpeg_debug') {
    ...
    if (!empty($_FILES['video']['tmp_name'])) {
        $file_info    = array(
            ...
            'allowed' => 'mp4,mov,webm,mpeg,3gp,avi,flv,ogg,mkv,mk3d,mks,wmv,m4v'
        );
...
if ($first == 'test_yandex_s3') {
    if ($pt->config->yandex_s3 == 'off' || empty($pt->config->yandex_s3_key) || empty($pt->config->yandex_s3_s_key)) {
    ...
    try {
        ...
        $buckets  = $s3Client->listBuckets();
        if (!empty($buckets)) {
            if ($s3Client->doesBucketExist($pt->config->yandex_bucket_name)) {
                $data['status'] = 200;
                $array          = array(
                    'upload/photos/d-cover.jpg',
                    'upload/photos/d-avatar.jpg',
...
if ($first == 'test_yandex_s3_2') {
    if ($pt->config->yandex_s3_2 == 'off' || empty($pt->config->yandex_s3_key_2) || empty($pt->config->yandex_s3_s_key_2)) {
        $data['message'] = 'Please enable Yandex Cloud Storage and fill all fields.';
    }
    ...
    try {
        ...
        $buckets  = $s3Client->listBuckets();
        if (!empty($buckets)) {
            if ($s3Client->doesBucketExist($pt->config->yandex_bucket_name_2)) {
                $data['status'] = 200;
                $array          = array(
                    'upload/photos/d-cover.jpg',
                    'upload/photos/d-avatar.jpg',
